    ...
    def render_achievement_target_cards(owner_totals, target):
        ...
        for owner, (count, arr) in sorted(owner_totals.items(), key=lambda x: -x[1][1]):
            ...
            <div class="owner-card">
              <div class="initials">{initials}</div>
              <div class="name">{owner}</div>
              <div class="arr">{fmt_currency(arr)} <span style="font-weight:400;color:#8189B0;font-size:11px">/ {fmt_currency(target)}</span></div>
...
<style>
  :root {{ --navy:#7B8FFF; --navy-light:#242A52; --gold:#E0AC4F; --slate:#C4CAE4; --bg:#0D1024; --ice:#2B3164; --border:rgba(255,255,255,0.08); --white:#FFFFFF; --green:#4ECB4E; --red:#F0716D; --purple:#B39CF0; --card:#171B3A; --heading:#AEB9FF; --muted:#8189B0; }}
  * {{ box-sizing: border-box; }}
  ...
  header {{ background:linear-gradient(135deg, #171B3A 0%, #232A5C 50%, #2E2258 100%); color:white; padding:28px 32px; box-shadow:0 4px 20px rgba(0,0,0,0.4); }}
  ...
  header .meta {{ font-size:13px; color:#B7BEE8; }}
  nav {{ display:flex; gap:4px; background:var(--navy-light); padding:0 32px; overflow-x:auto; box-shadow:0 2px 8px rgba(0,0,0,0.25); }}
  nav button {{ background:none; border:none; color:#9BA3D4; padding:15px 18px; font-size:13.5px; font-weight:600; cursor:pointer; border-bottom:3px solid transparent; white-space:nowrap; transition:all 0.2s; }}
  ...
  h2 {{ font-family:Georgia,serif; color:var(--heading); font-size:19px; margin:22px 0 12px; display:flex; align-items:center; gap:8px; }}
  .stat-row {{ display:flex; gap:14px; flex-wrap:wrap; margin-bottom:18px; }}
  .stat-card {{ background:var(--card); border-radius:12px; box-shadow:0 3px 12px rgba(0,0,0,0.3); padding:15px 18px; flex:1; min-width:130px; border-left:4px solid var(--navy); transition:transform 0.2s; border-top:1px solid var(--border); border-right:1px solid var(--border); border-bottom:1px solid var(--border); }}
  ...
  .owner-card {{ background:var(--card); border-radius:12px; box-shadow:0 3px 12px rgba(0,0,0,0.3); padding:13px 17px; flex:1; min-width:150px; border:1px solid var(--border); }}
  ...
  table {{ width:100%; border-collapse:collapse; background:var(--card); border-radius:12px; overflow:hidden; box-shadow:0 3px 12px rgba(0,0,0,0.25); font-size:12.5px; margin-bottom:18px; border:1px solid var(--border); }}
  th {{ background:linear-gradient(90deg,#20255499,#2E2258); color:white; text-align:left; padding:9px 12px; font-size:11.5px; }}
  td {{ padding:7px 12px; border-bottom:1px solid var(--border); color:var(--slate); }}
  tr.total-row td {{ font-weight:700; background:rgba(224,172,79,0.12); color:var(--gold); }}
  .num-cell {{ text-align:right; }}
  .center-cell {{ text-align:center; }}
  .empty-state {{ background:var(--card); border-radius:12px; padding:28px; text-align:center; color:var(--muted); margin-bottom:18px; border:1px solid var(--border); }}
  .empty-state .icon {{ font-size:28px; margin-bottom:6px; }}
  .badge {{ display:inline-block; background:var(--gold); color:#1A1200; font-size:10.5px; font-weight:700; padding:2px 8px; border-radius:12px; margin-left:8px; }}
  .chart-card {{ background:var(--card); border-radius:14px; box-shadow:0 3px 14px rgba(0,0,0,0.35); padding:20px 22px; margin-bottom:20px; position:relative; overflow:hidden; border:1px solid var(--border); }}
  ...
  .progress-wrap {{ background:rgba(255,255,255,0.06); border-radius:20px; height:28px; overflow:hidden; margin:10px 0; }}
  .progress-fill {{ background:linear-gradient(90deg, var(--gold), #E8B85C); height:100%; display:flex; align-items:center; justify-content:flex-end; padding-right:10px; color:#1A1200; font-size:11.5px; font-weight:700; transition:width 0.8s ease; }}
  ...
  footer {{ text-align:center; padding:20px; font-size:11.5px; color:var(--muted); }}
</style>
</head>
<body>
<header>
...
<script>
  ...
  Chart.defaults.color = '#9BA3D4';
  Chart.defaults.borderColor = 'rgba(255,255,255,0.08)';
  Chart.defaults.font.family = "-apple-system, BlinkMacSystemFont, 'Segoe UI', Calibri, sans-serif";

  const CHART_DATA = {chart_data_json};
  const NAVY = '#7B8FFF', GOLD = '#E0AC4F', ICE = '#3A4488', SLATE = '#C4CAE4', GREEN='#4ECB4E', RED='#F0716D', PURPLE='#B39CF0';
  ...
  new Chart(document.getElementById('ownerChart'), {{
    ...
    data: {{
      ...
      datasets: [
        {{ label: 'Achieved', data: CHART_DATA.byOwner.values, backgroundColor: [GOLD, NAVY, PURPLE, '#6E9BE0'], borderRadius: {{topLeft:8,bottomLeft:8,topRight:0,bottomRight:0}}, stack: 's' }},
        {{ label: 'Remaining to ₹2Cr', data: CHART_DATA.byOwner.remaining, backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: {{topLeft:0,bottomLeft:0,topRight:8,bottomRight:8}}, stack: 's' }}
      ]
    }},
    options: {{
      indexAxis: 'y',
  ...
  const leadColors = [RED, '#8189B0', GOLD, '#4E5488', GREEN];
  const leadTooltip = {{
    callbacks: {{
      label: (ctx) => {{
